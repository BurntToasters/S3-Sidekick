...
All release hosts must use Node.js `>= 24.18.0`, npm `12.0.2`, Rust `1.98.0`, and `cargo-audit 0.22.2`. The Flatpak manifest uses the Node 24 SDK extension and fails unless its in-sandbox Node/npm versions exactly match those signed coordinator pins. `release:supply-chain` provisions the pinned advisory scanner itself: the first run compiles it (idempotent afterwards, like the CI job). To pre-install it explicitly on a host, run:
