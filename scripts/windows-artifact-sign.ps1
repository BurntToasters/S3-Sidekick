...
# Idempotent for Tauri's signCommand: the bundler patches the package-type
# marker (invalidating any prior signature), then invokes this script for the
# staged runtime, the NSIS uninstaller (!uninstfinalize), and the installer.
# Re-signing an already-valid binary is unnecessary work and changes its
# timestamp, so leave those bytes untouched.
try {
