...
export function hasMinisignEnvelope(value) {
...
export function validateManifest(filePath) {
...
function main(argv = process.argv.slice(2)) {
  const files = argv.filter(Boolean);
  if (files.length === 0) {
    fail(
      "no updater manifests provided; pass latest-*.json paths as arguments",
    );
    process.exit(1);
  }
